...
const setupAtlas = async () => {
  try {
    console.log('🚀 Bắt đầu thiết lập MongoDB Atlas...');
    
    // Check if MONGODB_URI is set
    if (!process.env.MONGODB_URI || process.env.MONGODB_URI.includes('localhost')) {
      console.log('❌ Vui lòng cấu hình MONGODB_URI trong file config.env');
      console.log('📝 Ví dụ: MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/icdayroi');
      process.exit(1);
    }
    ...
    console.log(`✅ Đã thêm ${products.length} sản phẩm vào Atlas`);

    // Show some statistics
    const totalProducts = await Product.countDocuments();
    const categories = await Product.distinct('category');
    
    console.log('\n📊 Thống kê MongoDB Atlas:');
    console.log(`- Tổng số sản phẩm: ${totalProducts}`);
    console.log(`- Số danh mục: ${categories.length}`);
    console.log(`- Danh mục: ${categories.join(', ')}`);
    
    console.log('\n🎉 Thiết lập MongoDB Atlas hoàn tất!');
    console.log('💡 Bạn có thể chạy backend server với: npm run dev');
    
    process.exit(0);
  } catch (error) {
    console.error('❌ Lỗi khi thiết lập MongoDB Atlas:', error);
    console.log('\n💡 Hướng dẫn khắc phục:');
    console.log('1. Kiểm tra MONGODB_URI trong config.env');
    console.log('2. Đảm bảo IP address được whitelist trong Atlas');
    console.log('3. Kiểm tra username/password có đúng không');
    process.exit(1);
  }
};
